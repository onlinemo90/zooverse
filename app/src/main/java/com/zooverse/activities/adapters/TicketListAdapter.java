...
public class TicketListAdapter extends RecyclerView.Adapter<TicketListAdapter.TicketListViewHolder> {
	...
	public void onBindViewHolder(@NonNull TicketListViewHolder viewHolder, int position) {
		...
		if (Model.getStoredTickets().get(position).isForToday()) { // highlight if it is a ticket for today
			viewHolder.cardView.setCardBackgroundColor(Theme.getColor(R.attr.themeColorPrimary));
			viewHolder.ticketLabelTextView.setTextColor(Theme.getColor(R.attr.themeColorBackground));
			...
			viewHolder.ticketIcon.setColorFilter(Theme.getColor(R.attr.themeColorBackground));
		} else {
			...
			Theme.apply(viewHolder.cardView);
		}
	}
